...
def _engine_is_stale() -> bool:
    """
    Return True if any .cpp/.hpp source is newer than the compiled binary,
    meaning the binary must be recompiled to pick up source changes.

    """
    if not _ENGINE_BIN.exists():
        return True
    bin_mtime = _ENGINE_BIN.stat().st_mtime
    sources = _CPP_SOURCES + [
        str(_ENGINE_DIR / "linked_list.h"),
        str(_ENGINE_DIR / "tree.h"),
    ]
    for src in sources:
        try:
...
def init_engine() -> bool:
    """
    Compile the C++ engine if the binary is missing OR out of date.

    Recompiles automatically when any source file is newer than the binary,
    so you never run a stale engine after editing the C++ code.
    ...
    # Remove old binary so a failed compile cannot leave a stale one behind
    try:
...
def write_input(action: dict) -> bool:
    ...
    """
    try:
        with open(str(_INPUT_JSON), "w") as f:
...
def reset_game() -> dict | None:
    ...
    Writes {"action": "reset"} to input.json and deletes state.json so
    the engine triggers a full re-initialisation.
    ...
    Space : O(1)
    """
    # Write reset action so engine knows to start fresh
    try:
        with open(str(_INPUT_JSON), "w") as f:
            json.dump({"action": "reset"}, f)
    except OSError as e:
        print(f"[bridge] Cannot write input.json: {e}", file=sys.stderr)
        return None

    # Delete state.json so engine detects a fresh start
    try:
